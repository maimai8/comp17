...
let rec g expr = match expr with
  ...
  | Syntax.LetRec ((name, _), args, arg1, arg2) ->
    LetRec ((name, Type.gen_type()), args, g(arg1), g(arg2))
  | Syntax.Application (Syntax.Variable(name), args) ->
    let vlst = List.map (fun a -> Gensym.f("app")) args in
    let vlst_t = List.map (fun a -> (a, Type.gen_type())) vlst in
    let klst = List.map (fun expr -> g expr) args in
    let app = Application (name, vlst) in
    List.fold_right2 (fun a b c -> Let (a, b, c)) vlst_t klst app
